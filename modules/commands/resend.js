...
module.exports.handleEvent = async function ({ event, api, client, Users }) {
 ...
 let vip = global.config.UNSENTVIP;
  if (senderID == global.data.botID) return;
    ...
    if((event.type == "message_unsend") && !vip.includes(event.senderID)) {
      var getMsg = global.logMessage.get(messageID);
      if(!getMsg) return;
     let name = await Users.getNameUser(senderID);
      if(getMsg.attachment[0] == undefined) return api.sendMessage(`${name} remove this message: ${getMsg.msgBody}`,threadID)
      else {
            let num = 0
            let msg = {
              body:`${name} just removed ${getMsg.attachment.length} Attachments.${(getMsg.msgBody != "") ? `\n\content: ${getMsg.msgBody}` : ""}`,
              attachment:[],
...
module.exports.run = async function({ api, event, Threads }) {
	...
	return api.sendMessage(`Succesfully ${(data["resend"] == true) ? "turned on" : "Turned off"} resend!`, threadID, messageID);
}
