    ...
    switch (args[0]) {
        ...
        default: {
            if (permssion != 2) {
                ...
                try {
                    if (event.type != "message_reply") return api.sendMessage("You have not replied to the message to be alerted.", threadID, messageID);
                    if (event.messageReply.senderID == api.getCurrentUserID()) return api.sendMessage('Cant alert bot account.', threadID, messageID);
                    ...
                    return api.sendMessage(`Warned ${name} with reason: ${args.join(" ")}, ${(data.banned) ? `Because I was warned 3 times, the above account has been banned` : `The above account also ${data.warningLeft} warnings!`}`, threadID, messageID);
